...
fn test_packages_add_writes_to_config() {
    let home = setup_home_with_packages();
    Command::cargo_bin("heimdal").unwrap()
        .args(&["packages", "add", "ripgrep", "--manager", "apt", "--no-install"])
        .env("HOME", home.path())
        .assert().success();

    let dotfiles = home.path().join(".dotfiles");
    let content = std::fs::read_to_string(dotfiles.join("heimdal.yaml")).unwrap();
    assert!(content.contains("ripgrep"), "ripgrep not found in heimdal.yaml:\n{}", content);
}
...
fn test_packages_add_duplicate_is_ok() {
    ...
    assert!(count <= 3, "git appears too many times ({}), may be duplicated:\n{}", count, content);
}
...
fn test_packages_remove_updates_config() {
    ...
    assert!(!content.contains("- vim"), "vim still in heimdal.yaml:\n{}", content);
}
...
fn test_packages_suggest_detects_rust() {
    ...
    project_dir.child("Cargo.toml").write_str("[package]\nname = \"test\"\n").unwrap();

    Command::cargo_bin("heimdal").unwrap()
        .args(&["packages", "suggest", "--dir", project_dir.path().to_str().unwrap()])
        .env("HOME", home.path())
        .assert().success()
