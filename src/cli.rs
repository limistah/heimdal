...
pub enum Commands {
    ...
    Commit(CommitArgs),
    /// Profile management
    Profile { #[command(subcommand)] action: ProfileCmd },
    /// Package management
    Packages { #[command(subcommand)] action: PackagesCmd },
    /// Template operations
    Template { #[command(subcommand)] action: TemplateCmd },
    /// Secret management
    Secret { #[command(subcommand)] action: SecretCmd },
    ...
    /// State management
    State { #[command(subcommand)] action: StateCmd },
    /// Background sync management
    AutoSync { #[command(subcommand)] action: AutoSyncCmd },
}
...
pub struct ImportArgs {
    #[arg(short, long, default_value = "~/.dotfiles", help = "Path to existing dotfiles")]
    pub path: String,
    #[arg(short, long, help = "Tool to import from (stow, dotbot, chezmoi, yadm, homesick, auto)")]
...
pub enum ProfileCmd {
    ...
    Switch { name: String, #[arg(long)] no_apply: bool },
    /// List all profiles
    List,
    /// Show profile details
    Show { name: Option<String>, #[arg(short, long)] resolved: bool },
    /// Show current profile
    Current,
    /// Create a new profile
    Create { name: String, #[arg(long)] extends: Option<String> },
    /// Clone an existing profile
    Clone { source: String, dest: String },
    /// Compare two profiles
    Diff { profile1: Option<String>, profile2: String },
}

#[derive(Subcommand)]
pub enum PackagesCmd {
    /// Add a package
    Add { name: String, #[arg(short, long)] manager: Option<String>, #[arg(long)] no_install: bool },
    /// Remove a package
    Remove { name: String, #[arg(long)] no_uninstall: bool },
    /// List packages
    List { #[arg(short, long)] installed: bool },
    /// Search packages
    Search { query: String },
    /// Suggest packages based on project files
    Suggest { #[arg(short, long)] dir: Option<String> },
    /// Show package info
    Info { name: String },
    /// List package groups
    Groups,
...
pub enum TemplateCmd {
    ...
    Preview { src: String, #[arg(short, long)] profile: Option<String> },
    /// List all templates
    List,
    /// Show available variables
    Variables { #[arg(short, long)] profile: Option<String> },
}
...
pub enum SecretCmd {
    ...
    Get { name: String },
    /// Remove a secret
    Remove { name: String, #[arg(short, long)] force: bool },
    /// List secret names
    List,
}
...
pub enum StateCmd {
    ...
    LockInfo,
    /// Force-remove lock
    Unlock { #[arg(short, long)] force: bool },
    /// Check for file drift
    CheckDrift,
    /// Check for state conflicts
    CheckConflicts,
    /// Show operation history
    History { #[arg(short, long, default_value = "10")] limit: usize },
}

#[derive(Subcommand)]
pub enum AutoSyncCmd {
    /// Enable background sync
    Enable { #[arg(short, long, default_value = "1h")] interval: String },
